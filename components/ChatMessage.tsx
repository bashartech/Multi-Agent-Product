...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  useEffect(() => {
    // Format time only on client-side to prevent hydration errors
    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-4`}>
      <div
        className={`max-w-xs lg:max-w-md px-4 py-3 rounded-lg shadow-md transition-all duration-200 ${
          isUser
            ? "bg-red-600 text-white shadow-lg shadow-red-600/30 rounded-br-none"
            : "bg-slate-800 text-gray-100 border border-slate-700 shadow-lg shadow-slate-800/50 rounded-bl-none"
        }`}
      >
        <p className="text-sm break-words">{message.text}</p>
        <span className={`text-xs mt-2 block opacity-70 ${isUser ? "text-red-100" : "text-gray-400"}`}>
          {formattedTime}
        </span>
      </div>
    </div>
